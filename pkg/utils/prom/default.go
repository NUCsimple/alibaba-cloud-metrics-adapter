...
func DefaultConfig(rateInterval time.Duration, labelPrefix string) *MetricsDiscoveryConfig {
	return &MetricsDiscoveryConfig{
		Rules: []DiscoveryRule{
			...
			{
				SeriesQuery: string(MatchSeries("", NameMatches("^container_.*"), LabelNeq("container_name", "POD"), LabelNeq("namespace", ""), LabelNeq("pod_name", ""))),
			...
			{
				SeriesQuery:   string(MatchSeries("", LabelNeq(fmt.Sprintf("%snamespace", labelPrefix), ""), NameNotMatches("^container_.*"))),
